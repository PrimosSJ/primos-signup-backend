...
def submit(_, payload: RegisterForm):
    try:
      ...
      free_blocks = sum(
              [False if (i % 9 == 0 or (i + 1) % 9 == 0) else not payload.bussy_schedule[i] for i in range(45)]
      )

      if (free_blocks < 6):
          return 400, {'detail': 'there must be at least 6 free blocks, not counting 1-2 and 15-16'}
